...
// Pada baris ke-12 (dashboard/users.php), tambahkan proteksi role untuk proses Hapus
if (isset($_GET['delete_id'])) {
    if ($_SESSION['role'] !== 'admin') {
        die("Akses Ditolak: Hanya admin yang diizinkan menghapus pengguna.");
    }
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-x-auto custom-scrollbar">
                <table class="w-full text-left border-collapse min-w-[700px]">
                    ...
                    <tbody class="divide-y">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="p-4 text-center">
                                        <div class="flex justify-center gap-2">
                                            <a href="user-form.php?id=<?= $row['id'] ?>"
                                                class="bg-blue-50 text-blue-600 px-3 py-1 rounded text-sm font-medium transition hover:bg-blue-100"><i
                                                    class="fas fa-edit"></i> Edit</a>
                                            <?php if ($row['id'] != $_SESSION['user_id'] && $_SESSION['role'] === 'admin'): ?>
                                                <button onclick="openDeleteModal(<?= $row['id'] ?>)"
                                                    class="bg-red-50 text-red-600 px-3 py-1 rounded text-sm font-medium transition hover:bg-red-100"><i
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="5" class="p-4 text-center text-gray-500">Belum ada data pengguna.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
